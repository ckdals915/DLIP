...
int hmin = 20, hmax = 40, smin = 100, smax = 255, vmin = 100, vmax = 255;
...
int main()
{
	Mat image_disp, hsv, hue, mask, src2, hsv_scale, hsv_gray;
	vector<Mat> hsv_split;
	vector<Mat> bgr_split;
	vector<vector<Point> > contours;

	
	src = imread("..//..//..//Images//rgbnew.jpg", IMREAD_COLOR);
	src2 = imread("..//..//..//Images//gray.jpg", IMREAD_GRAYSCALE);
	...
	while (true)
	{
		src2.copyTo(image_disp);


		imshow("Source", src);
		//imshow("Source_gray", src2);
		/******** Convert BGR to HSV ********/
		// input mat: image
		// output mat: hsv
		cvtColor(src, hsv, COLOR_BGR2HSV);
		
		cvtColor(src, hsv_gray, COLOR_BGR2GRAY);
		/******** Add Pre-Processing such as filtering etc  ********/
		// split the Matrix
		plotHist(hsv_gray, "GRAY", 800, 800);
		split(hsv, hsv_split);
		//plotHist(hsv, "HSV", 800, 800);
		//imshow("H", hsv_split[0]);
		//imshow("S", hsv_split[1]);
		//imshow("V", hsv_split[2]);

		//plotHist(hsv_split[0], "H", 800, 800);
		//plotHist(hsv_split[1], "S", 800, 800);
		//plotHist(hsv_split[2], "V", 800, 800);

		//equalizeHist(hsv_split[0], hsv_split[0]);
		//equalizeHist(hsv_split[1], hsv_split[1]);
		//equalizeHist(hsv_split[2], hsv_split[2]);

		medianBlur(hsv_split[2], hsv_split[2], 5);
		plotHist(hsv_split[2], "V_median", 800, 800);
		//GaussianBlur(hsv_split[2], hsv_split[2], Size(9, 9), 0, 0);
		//blur(hsv_split[2], hsv_split[2], Size(9, 9));

		//imshow("hsv_scale", hsv_split[2]);
