...
int main()
{
	...
	image = imread("..//..//..//Images//groupimage_crop.jpg");
...
static void onMouse(int event, int x, int y, int, void*)
{
	if (selectObject)  // for any mouse motion
	{
		...
		selection &= Rect(0, 0, image.cols, image.rows);  /// Bitwise AND  check selectin is within the image coordinate 안전장치(image 내에서만 드래그 할 수 있도록)
	}
